...
function formChecker1() {
    ...
    if(ageInput.value.length < 1 || ageInput.value.length > 2 || ageInput.value < 18 || ageInput.value > 45) {
        alert("Vous êtes trop jeune, ou trop vieux (déso) !");
        return;
    }

    if(sexSelect.value == "femme" || sexSelect.value == "homme" || sexSelect.value == "secret") {
    } else {
...
function formChecker2() {
    if(regionSelect.value == "Auvergne-Rhone-Alpes"
    ...
    if(hourSelect.value == "matin" || hourSelect.value == "apresMidi" || hourSelect.value == "soir" || hourSelect.value == "nuit") {
    } else {
        alert("Merci ce selectionner une disponibilité horaire !");
        return;
    }

    if(workSelect.value == "securite" || workSelect.value == "bar" || workSelect.value == "technique" || workSelect.value == "animation") {
    } else {
